...
const PlatformIcons = {
  xiaohongshu: ({ size = 24 }: { size?: number }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
      <text x="50%" y="70%" fontSize="12" fontWeight="bold" textAnchor="middle" fill="white">红</text>
    </svg>
  ),
  linkedin: ({ size = 24 }: { size?: number }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
...
export function IntentCard({ intent, showStatus = false, onUpdateStatus, onUpdateNote }: IntentCardProps) {
  ...
  const PlatformIcon = PlatformIcons[intent.platform] || (() => null)
    ...
    <div className="relative group">
      <div className="rounded-2xl p-6 relative h-full flex flex-col bg-white/90 backdrop-blur-sm border border-gray-200/50 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
        <div className="flex justify-between items-start mb-5">
          <div className="flex items-center gap-3">
            <img
              ...
              alt={intent.author}
              className="w-12 h-12 rounded-full border-2 border-gray-200 object-cover shadow-sm bg-gray-50"
            />
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-gray-900 text-[15px] truncate">{intent.author || "Unknown User"}</p>
              <p className="text-xs text-gray-500">{intent.timeAgo}</p>
            </div>
          </div>
          <div
            className={`${platformConfig.bg} ${platformConfig.textColor} w-10 h-10 rounded-xl flex items-center justify-center shadow-md transform group-hover:scale-110 transition-all duration-300 border-2 border-white`}
          >
        ...
        {showStatus && (
          <div className="mb-4">
            <DropdownMenu>
              <DropdownMenuTrigger className="flex items-center gap-2 px-2 py-1 bg-white/80 border border-gray-200 rounded-lg text-xs font-semibold shadow-sm hover:bg-white transition-colors capitalize focus:outline-none">
        ...
        <div className="mb-5 flex-1">
          <button
            ...
            className="text-left w-full hover:bg-gray-50/80 -mx-2 px-2 py-2 rounded-lg transition-colors group/text"
          >
            <p className="text-sm text-gray-700 leading-relaxed line-clamp-4 group-hover/text:text-gray-900">
              {intent.content}
            </p>
          </button>

          {intent.topComment && (
            <div className="mt-4 p-3 bg-purple-50/30 rounded-xl border border-purple-100/50">
              <div className="flex items-center gap-2 mb-1">
                <MessageSquare size={12} className="text-purple-500" />
                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Top Comment</span>
              </div>
        ...
        {showStatus && (
          <form onSubmit={handleAddNote} className="mb-5 flex gap-2">
            <input 
              ...
              className="flex-1 bg-gray-50 text-[11px] border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-purple-500/30 transition-all"
            />
            <button type="submit" className="p-2 bg-purple-100 text-purple-600 rounded-lg hover:bg-purple-600 hover:text-white transition-all">
              <Plus size={14} />
            </button>
          </form>
        )}
        ...
        <div className="mt-auto space-y-2.5">
          <div className="flex justify-between items-center">
            <span className="text-[11px] font-medium text-gray-500">AI Intent Score</span>
        ...
        <div className="flex gap-2.5 mt-5 pt-5 border-t border-gray-100">
          <button
            onClick={handleViewPost}
            className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-purple-700 text-white hover:from-purple-700 hover:to-purple-800 rounded-lg py-2.5 transition-all text-xs font-semibold shadow-sm hover:shadow-md"
          >
          ...
          {!showStatus && (
            <button
              onClick={handleSave}
              className={`flex-1 flex items-center justify-center gap-2 rounded-lg py-2.5 transition-all text-xs font-semibold ${
                isSaved
