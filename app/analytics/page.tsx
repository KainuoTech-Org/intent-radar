...
export default function AnalyticsPage() {
  const stats = [
    { label: "Total Leads", value: "1,234", change: "+12%", icon: Zap },
    { label: "Conversion Rate", value: "42.9%", change: "+8%", icon: TrendingUp },
    { label: "Revenue Generated", value: "$89K", change: "+15%", icon: BarChart3 },
    { label: "Avg Intent Score", value: "84.5", change: "+19%", icon: Users },
  ]
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-[14px] p-8 border border-[#E5E1D8] shadow-sm">
            ...
            <div className="h-[300px] w-full flex items-end justify-between gap-4 border-b border-[#E5E1D8] pb-2 relative">
              {/* Dummy Bars representing the chart from Figma */}
              {[40, 60, 45, 80, 70, 90, 85].map((h, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-2 group">
                  <div className="w-full bg-[#F5F3F0] rounded-t-sm group-hover:bg-[#EDE9E0] transition-colors relative" style={{ height: `${h}%` }}>
          ...
          <div className="bg-white rounded-[14px] p-8 border border-[#E5E1D8] shadow-sm">
            ...
            <div className="space-y-6">
              {[
                { name: "LinkedIn", count: 456, color: "bg-[#0A66C2]" },
                { name: "Xiaohongshu", count: 392, color: "bg-[#FF2442]" },
                { name: "Facebook", count: 234, color: "bg-[#1877F2]" },
                { name: "X (Twitter)", count: 165, color: "bg-[#000000]" },
              ].map((p) => (
                <div key={p.name}>
                  ...
                  <div className="w-full bg-[#F5F3F0] h-2 rounded-full overflow-hidden">
                    <div className={`h-full ${p.color}`} style={{ width: `${(p.count / 500) * 100}%` }}></div>
                  </div>
                </div>
              ))}
            </div>
